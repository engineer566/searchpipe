...
> **⚠️ 占位提醒**：以下检查点按需求拟定，具体路由/字段以功能实现代码为准；合并实现后必须对照 `docs/INDEX.md` 路由速查核对并修订本节。

- [ ] 管理员向**指定用户**发送站内信 → 目标用户可见，其他用户不可见
- [ ] 管理员发送**全局通知** → 全部用户在列表可见
- [ ] 用户端 `/dashboard/messages`（占位路由，以实际实现为准）→ 200，消息列表正确、按时间排序
- [ ] 未读数：导航/入口未读角标数与实际未读一致；点开消息后未读数减少
- [ ] 未登录访问消息页 → 跳登录；普通用户访问管理端发信入口 → 被拒
- [ ] 反馈工单联动：管理员处理/关闭工单时通知用户（若实现）→ 用户收到对应站内信
